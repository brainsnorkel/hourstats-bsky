...
### Fixed
- **CRITICAL**: Added early-stop logic to fetcher to prevent timeout and ensure posts are made. Fetcher now stops at 14 minutes elapsed time if it has collected ≥1000 posts, leaving 1 minute buffer before the 15-minute Lambda timeout to ensure processor dispatch. This prevents fetcher from timing out and ensures reports are always posted even when fetching takes longer than expected.
