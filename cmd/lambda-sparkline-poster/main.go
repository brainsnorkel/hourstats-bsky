...
func (h *SparklinePosterHandler) HandleRequest(ctx context.Context, event StepFunctionsEvent) (Response, error) {
	...
	if runState.TopPostURI != "" && runState.TopPostCID != "" {
		...
		if _, _, err := blueskyClient.PostWithImageAsReply(ctx, postText, imageData, altText, runState.TopPostURI, runState.TopPostCID, runState.TopPostURI, runState.TopPostCID); err != nil {
