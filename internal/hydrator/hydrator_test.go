...
type updateCall struct {
	URI, AuthorHandle, Sentiment string
...
func (m *mockUpdater) UpdatePostEngagement(_ context.Context, uri string, likes, reposts, replies int, authorHandle, sentiment string, engagementScore float64) error {
	m.mu.Lock()
	m.calls = append(m.calls, updateCall{
		URI: uri, Likes: likes, Reposts: reposts, Replies: replies,
		AuthorHandle: authorHandle, Sentiment: sentiment, EngagementScore: engagementScore,
	})
	m.mu.Unlock()
	return m.err
}
...
func TestHydrateUpdaterCalled(t *testing.T) {
	...
	if c.Sentiment != "" {
		t.Errorf("Sentiment = %q, want empty", c.Sentiment)
	}
	if c.EngagementScore != 0 {
		t.Errorf("EngagementScore = %f, want 0", c.EngagementScore)
	}
}
